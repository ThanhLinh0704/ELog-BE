...
class ManifestServiceImplTest {
    ...
    private OrderItem item(int n, Store store){
        Order order = Order.builder().id((long)n).store(store).build();
        BigDecimal weight=BigDecimal.valueOf(n);
        BigDecimal volume=BigDecimal.valueOf(n,2);
        return OrderItem.builder().id((long)n).order(order).product(Product.builder().id((long)n).sku("SKU-"+n).productName("Product "+n).build()).quantity(n).unitWeightKg(BigDecimal.ONE).unitVolumeM3(new BigDecimal("0.01")).lineWeightKg(weight).lineVolumeM3(volume).build();
    }
    private List<ManifestLine> captureGenerated(List<TripDraftStop> stops, java.util.function.Function<TripDraftStop,List<OrderItem>> items){
        when(tripDraftRepo.findById(1L)).thenReturn(Optional.of(draft));
        when(tripDraftStopRepo.findByTripDraftIdAndIsActiveTrueOrderBySequenceNoAsc(1L)).thenReturn(stops);
        when(userRepo.findByUsername("warehouse")).thenReturn(Optional.of(user));
        List<OrderItem> allItems = new ArrayList<>();
        for(TripDraftStop s:stops) allItems.addAll(items.apply(s));
    ...
    void fourStopsGenerateReversedLifo(){List<TripDraftStop> stops=IntStream.rangeClosed(1,4).mapToObj(this::stop).toList();List<ManifestLine> lines=captureGenerated(stops,s->List.of(item(s.getSequenceNo(), s.getStore())));
    ...
    void manifestPreservesItemBreakdown(){TripDraftStop stop=stop(1);List<OrderItem> items=IntStream.rangeClosed(1,5).mapToObj(i->item(i, stop.getStore())).toList();List<ManifestLine> lines=captureGenerated(List.of(stop),s->items);assertEquals(5,lines.size());
    ...
    @Test @DisplayName("[L1-MF-08] duplicate manifest generation is rejected")
    void duplicateManifestIsRejected(){when(tripDraftRepo.findById(1L)).thenReturn(Optional.of(draft));when(manifestRepo.existsByTripDraftId(1L)).thenReturn(true);BusinessException e=assertThrows(BusinessException.class,()->service.generateManifest(1L,"warehouse"));assertEquals(ErrorCode.MANIFEST_ALREADY_EXISTS,e.getErrorCode());}

    @Test @DisplayName("[L1-MF-09] draft without active stops rejects manifest generation")
    void emptyActiveStopsRejectsGeneration(){when(tripDraftRepo.findById(1L)).thenReturn(Optional.of(draft));when(manifestRepo.existsByTripDraftId(1L)).thenReturn(false);when(tripDraftStopRepo.findByTripDraftIdAndIsActiveTrueOrderBySequenceNoAsc(1L)).thenReturn(List.of());BusinessException e=assertThrows(BusinessException.class,()->service.generateManifest(1L,"warehouse"));assertEquals(ErrorCode.NO_ACTIVE_STOP,e.getErrorCode());}

    @Test
    @DisplayName("[L1-MF-10] getManifestByStop groups lines by stop sequence")
    void getManifestByStopSuccess() {
        Manifest manifest = Manifest.builder().manifestId(10L).tripDraft(draft).generatedBy(user).generatedAt(LocalDateTime.now()).totalLines(2).totalWeightKg(BigDecimal.TEN).totalVolumeM3(BigDecimal.ONE).build();
        ManifestLine line1 = ManifestLine.builder().lifoSequence(1).stopSequenceNo(2).storeCode("S2").storeName("Store 2").productCode("SKU-2").quantity(2).lineWeightKg(BigDecimal.valueOf(5)).lineVolumeM3(BigDecimal.valueOf(0.5)).build();
        ManifestLine line2 = ManifestLine.builder().lifoSequence(2).stopSequenceNo(1).storeCode("S1").storeName("Store 1").productCode("SKU-1").quantity(1).lineWeightKg(BigDecimal.valueOf(5)).lineVolumeM3(BigDecimal.valueOf(0.5)).build();

        when(manifestRepo.findByTripDraftIdWithDetails(1L)).thenReturn(Optional.of(manifest));
        when(manifestLineRepo.findByManifestManifestIdOrderByStopSequenceNoAscLifoSequenceAsc(10L)).thenReturn(List.of(line2, line1));

        var resp = service.getManifestByStop(1L);
        assertNotNull(resp);
        assertEquals(10L, resp.getManifestId());
        assertEquals(2, resp.getStops().size());
    }
}
