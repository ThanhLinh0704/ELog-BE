...
class UserServiceImplTest {
    ...
    @Test @DisplayName("[L1-US-10] getUserById returns user response")
    void getUserByIdSuccess() {
        User user = User.builder().id(9L).username("stable").fullName("Stable User").build();
        when(userRepository.findById(9L)).thenReturn(Optional.of(user));
        when(userMapper.toResponse(user)).thenReturn(UserResponse.builder().id(9L).username("stable").build());
        UserResponse resp = service.getUserById(9L);
    ...
    @Test
    @DisplayName("[L1-US-11] updateUserRoles rejects admin self-demoting")
    void updateUserRolesRejectsAdminSelfDemotion() {
        User admin = User.builder().id(1L).username("admin").build();
        when(userRepository.findById(1L)).thenReturn(Optional.of(admin));

        com.elog.dto.request.user.UserRolesUpdateRequest req = new com.elog.dto.request.user.UserRolesUpdateRequest();
        req.setRoles(Set.of("DISPATCHER")); // Missing SYSTEM_ADMIN

        BusinessException ex = assertThrows(BusinessException.class, () -> service.updateUserRoles(1L, req, "admin"));
        assertEquals(ErrorCode.ACCESS_DENIED, ex.getErrorCode());
    }

    @Test
    @DisplayName("[L1-US-12] updateUserRoles saves new roles")
    void updateUserRolesSuccess() {
        User user = User.builder().id(2L).username("driver01").build();
        Role role = Role.builder().id(3L).name("DRIVER").build();

        when(userRepository.findById(2L)).thenReturn(Optional.of(user));
        when(roleRepository.findByName("DRIVER")).thenReturn(Optional.of(role));
        when(userRepository.save(any())).thenReturn(user);
        when(userMapper.toResponse(user)).thenReturn(UserResponse.builder().id(2L).roles(Set.of("DRIVER")).build());

        com.elog.dto.request.user.UserRolesUpdateRequest req = new com.elog.dto.request.user.UserRolesUpdateRequest();
        req.setRoles(Set.of("DRIVER"));

        UserResponse resp = service.updateUserRoles(2L, req, "admin");
        assertNotNull(resp);
        assertEquals(Set.of("DRIVER"), resp.getRoles());
    }

    @Test
    @DisplayName("[L1-US-13] getAllUsers returns paginated list")
    void getAllUsersSuccess() {
        ...
        var resp = service.getAllUsers("user", "DRIVER", true, org.springframework.data.domain.PageRequest.of(0, 10));
        assertNotNull(resp);
    ...
    @Test
    @DisplayName("[L1-US-14] updateUserStatus rejects admin self-deactivation")
    void updateUserStatusRejectsAdminSelfDeactivation() {
        User admin = User.builder().id(1L).username("admin").isActive(true).build();
        when(userRepository.findById(1L)).thenReturn(Optional.of(admin));

        com.elog.dto.request.user.UserStatusUpdateRequest req = new com.elog.dto.request.user.UserStatusUpdateRequest();
        req.setIsActive(false);

        BusinessException ex = assertThrows(BusinessException.class, () -> service.updateUserStatus(1L, req, "admin"));
        assertEquals(ErrorCode.ACCESS_DENIED, ex.getErrorCode());
    }

    @Test
    @DisplayName("[L1-US-15] updateUserStatus updates user active status")
    void updateUserStatusSuccess() {
        User user = User.builder().id(2L).username("user02").isActive(true).build();
        when(userRepository.findById(2L)).thenReturn(Optional.of(user));
        when(userRepository.save(any())).thenReturn(user);
        when(userMapper.toResponse(user)).thenReturn(UserResponse.builder().id(2L).isActive(false).build());

        com.elog.dto.request.user.UserStatusUpdateRequest req = new com.elog.dto.request.user.UserStatusUpdateRequest();
        req.setIsActive(false);

        UserResponse resp = service.updateUserStatus(2L, req, "admin");
        assertNotNull(resp);
